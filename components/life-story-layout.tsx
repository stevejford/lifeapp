...
export function LifeStoryLayout({ children, currentPage, hideYellowBar = false }: LifeStoryLayoutProps) {
  return (
    <div className="min-h-screen flex flex-col">
      ...
      <nav className="bg-white border-b w-full">
        <div className="flex justify-between items-center px-4 py-2">
          ...
          <div className="flex space-x-4 justify-center w-1/3">
            <Link href="/life-story" className={`text-sm font-medium ${currentPage === 'life-story' ? 'text-blue-600 border-b-2 border-blue-600' : 'text-gray-500 hover:text-gray-700'}`}>
              Life story
            </Link>
            <Link href="/biography" className={`text-sm font-medium ${currentPage === 'biography' ? 'text-blue-600 border-b-2 border-blue-600' : 'text-gray-500 hover:text-gray-700'}`}>
              Biography
            </Link>
          </div>
      ...
      {/* Yellow banner */}
      {!hideYellowBar && (
        <div className="bg-yellow-50 w-full py-2">
          <div className="flex justify-between items-center px-4 max-w-screen-xl mx-auto">
            <span className="text-yellow-800 font-medium">Free life story</span>
